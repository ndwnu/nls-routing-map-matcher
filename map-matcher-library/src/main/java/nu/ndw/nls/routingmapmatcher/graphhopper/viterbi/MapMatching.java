...
/**
 * This class is a fork of com.graphhopper.matching.MapMatching.java In the previous version there was an option to set
 * a custom extension of a distance calculator DistanceCalc. In the current version 7.0 this is prevented as the
 * distanceCalc attribute is private final. However, for NLS functionality this custom distance calculator is needed to
 * prevent filtering on input geometries. During testing unwanted discrepancies where found between the old and current
 * versions of this library. If we use the option of setting this custom distance calculator to prevent this filtering
...
public class MapMatching {
    ...
    public static Router routerFromGraphHopper(GraphHopper graphHopper, PMap hints) {
        ...
        BooleanEncodedValue inSubnetworkEnc = graphHopper.getEncodingManager()
                .getBooleanEncodedValue(Subnetwork.key(profileStr));
        ...
        Router router = new Router() {
            ...
            @Override
            public List<Path> calcPaths(QueryGraph queryGraph, int fromNode, int fromOutEdge, int[] toNodes,
                    int[] toInEdges) {
            ...
            private Path calcOnePath(QueryGraph queryGraph, int fromNode, int toNode, int fromOutEdge, int toInEdge) {
                ...
                if (landmarks != null) {
                    AStarBidirection aStarBidirection = new AStarBidirection(queryGraph, queryGraphWeighting,
                    ...
                    LMApproximator lmApproximator = LMApproximator.forLandmarks(queryGraph, queryGraphWeighting,
                            landmarks, activeLM);
                    ...
                    DijkstraBidirectionRef dijkstraBidirectionRef = new DijkstraBidirectionRef(queryGraph,
                            queryGraphWeighting, TraversalMode.EDGE_BASED) {
                        @Override
                        protected void initCollections(int size) {
                            super.initCollections(50);
                        }
    ...
    public MatchResult match(List<Observation> observations) {
        ...
        statistics.put("transitionDistances", seq.stream().filter(s -> s.transitionDescriptor != null)
                .mapToLong(s -> Math.round(s.transitionDescriptor.getDistance())).toArray());
        statistics.put("visitedNodes", router.getVisitedNodes());
        statistics.put("snapDistanceRanks",
                IntStream.range(0, seq.size()).map(i -> snapsPerObservation.get(i).indexOf(seq.get(i).state.getSnap()))
                        .toArray());
        statistics.put("snapDistances", seq.stream().mapToDouble(s -> s.state.getSnap().getQueryDistance()).toArray());
        statistics.put("maxSnapDistances", IntStream.range(0, seq.size()).mapToDouble(
                        i -> snapsPerObservation.get(i).stream().mapToDouble(Snap::getQueryDistance).max().orElse(-1.0))
                .toArray());
        ...
        result.setMatchMillis(seq.stream().filter(s -> s.transitionDescriptor != null)
                .mapToLong(s -> s.transitionDescriptor.getTime()).sum());
    ...
    /**
     * Filters observations to only those which will be used for map matching (i.e. those which are separated by at
     * least 2 * measurementErrorSigman
     */
    ...
    /**
     * Creates TimeSteps with candidates for the GPX entries but does not create emission or transition probabilities.
     * Creates directed candidates for virtual nodes and undirected candidates for real nodes.
     */
    private List<ObservationWithCandidateStates> createTimeSteps(List<Observation> filteredObservations,
            List<List<Snap>> splitsPerObservation) {
    ...
    private List<SequenceState<State, Observation, Path>> computeViterbiSequence(
        ...
        while (!q.isEmpty()) {
            ...
            final double linearDistance =
                    distanceCalc.calcDist(timeStep.observation.getPoint().lat, timeStep.observation.getPoint().lon,
            ...
            int[] toInEdges = nextTimeStep.candidates.stream().mapToInt(
                            to -> to.isOnDirectedEdge() ? to.getIncomingVirtualEdge().getEdge() : EdgeIterator.ANY_EDGE)
                    .toArray();
            ...
            for (int i = 0; i < nextTimeStep.candidates.size(); i++) {
                ...
                if (path.isFound()) {
                    ...
                    double minusLogProbability = qe.minusLogProbability - probabilities.emissionLogProbability(
                            to.getSnap().getQueryDistance()) - transitionLogProbability;
                    Label label1 = labels.get(to);
                    if (label1 == null || minusLogProbability < label1.minusLogProbability) {
                        q.stream().filter(oldQe -> !oldQe.isDeleted && oldQe.state == to).findFirst()
                                .ifPresent(oldQe -> oldQe.isDeleted = true);
                        Label label = new Label();
                        label.state = to;
        ...
        while (qe != null) {
            final SequenceState<State, Observation, Path> ss = new SequenceState<>(qe.state, qe.state.getEntry(),
                    qe.back == null ? null : roadPaths.get(new Transition<>(qe.back.state, qe.state)));
            result.add(ss);
            qe = qe.back;
        }
    ...
    private double gpxLength(List<Observation> gpxList) {
        if (gpxList.isEmpty()) {
            ...
            for (int i = 1; i < gpxList.size(); i++) {
                ...
                gpxLength += distanceCalc.calcDist(prevEntry.getPoint().lat, prevEntry.getPoint().lon,
                        entry.getPoint().lat, entry.getPoint().lon);
                prevEntry = entry;
            }
            return gpxLength;
        }
