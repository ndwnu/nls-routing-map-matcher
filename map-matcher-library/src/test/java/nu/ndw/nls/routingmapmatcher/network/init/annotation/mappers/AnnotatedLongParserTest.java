...
class AnnotatedLongParserTest {

    private static final int EDGE_ID = 1;
    private static final long EXPECTED_A = 2;
    private static final long EXPECTED_B = 3;
    private static final long INT_TOO_LARGE = 100;
    ...
    @Mock
    private EncodedValueDto<Link, Long> encodedValueDto;
    @Mock
    ...
    void handleWayTags_ok_oneValueForBothDirections() {
        ...
        EncodedLongMapper<Link> intParser = new EncodedLongMapper<>(encodedValueLookup, encodedValueDto);

        intParser.handleWayTags(EDGE_ID, egdeIntAccess, link, null);

        verify(intEncodedValue).setInt(false, EDGE_ID, egdeIntAccess, (int) EXPECTED_A);
    }
    ...
    void handleWayTags_ok_separateValuesPerDirection() {
        ...
        verify(intEncodedValue).setInt(false, EDGE_ID, egdeIntAccess, (int) EXPECTED_A);
        verify(intEncodedValue).setInt(true, EDGE_ID, egdeIntAccess, (int) EXPECTED_B);
    }
